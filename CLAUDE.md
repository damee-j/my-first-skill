...
- **daily-focus/** — Lark 봇이 매일 밤 10시에 DM으로 오늘의 회고(관찰/배움/실행/떠오른 생각)를 돕고, 내일 집중할 딱 한 가지 일을 정해서 Lark 캘린더에 Focus Block을 자동 생성. Lark (IM + 캘린더) + Gemini/OpenAI/Anthropic (AI) 연동. 설계서는 `SKILL.md` 참조.
- **recruitment-evaluation/** — 프로덕트 본부(PM, Product Designer) 채용 평가 자동화. 서류심사 → 45분 면접질문 생성 → 면접평가/합불 판정까지 3개 에이전트로 구성. `/recruit` 스킬로 통합 호출. 설계서는 `SKILL.md` 참조.
