...
- Part A 회고: Lark 회고 질문 4가지 → 사용자 응답 대기(5분) → Gemini Coach 피드백 → Lark 전달
- Part B 내일 Focus: Lark 질문 → 사용자 응답 대기(5분) → 스콥 분석 → **내일** 캘린더 빈 시간 탐색 → Focus Block 생성 → 요약 발송
...
- **scope_analyzer.py** — `analyze_scope()`. AI 폴백 체인: Gemini(`google-genai` SDK) → OpenAI → Anthropic → 키워드 휴리스틱. 항상 결과 반환
- **lark_calendar.py** — 일정 조회(`/events/instance_view`), 빈 시간 탐색, Focus Block CRUD
- **coach_gpt.py** — `get_coach_feedback()`. Gemini 2.5 Flash Lite REST API 직접 호출
...
| `GEMINI_API_KEY` | 스콥 분석 Primary + Coach 피드백 |
| `OPENAI_API_KEY` | 스콥 분석 Fallback |
| `ANTHROPIC_API_KEY` | 스콥 분석 Fallback |
...
**사용자 입력 포맷**: `"작업명 | 시간"` (예: `"PRD 초안 작성 | 4시간"`). `|` 없으면 AI가 시간 추정.
...
**Gemini API**: `generativelanguage.googleapis.com`. scope_analyzer는 `google-genai` SDK로 structured JSON output (`gemini-2.0-flash`), coach_gpt는 REST API로 호출 (`gemini-2.5-flash-lite`).
...
- 스케줄: 13:00 UTC (22:00 KST) 단일 cron
- 수동 실행: workflow_dispatch
- Secrets: `LARK_APP_ID`, `LARK_APP_SECRET`, `LARK_USER_TOKEN`, `LARK_USER_OPEN_ID`, `LARK_CHAT_ID`, `GEMINI_API_KEY`, `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`
